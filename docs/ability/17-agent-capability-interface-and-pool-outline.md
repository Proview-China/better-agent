...
以下内容作为 Gate 1 的“公共壳第一版冻结面”，只定义最小公共语言，不等于任何能力线闭环实现：

- 统一命名：`CapabilityKey = namespace.action` 是唯一稳定标识，`capability + action` 是结构化拆分形态，`operation` 仅作为历史别名使用。
- Support Matrix 字段口径：`plane`/`pool`/`weight`/`defaultLayer` + `providerSupport.{status, preferredLayer, notes}`，其余字段不进入首轮冻结口径。
- Request 最小公共形状：`provider`、`model`、`layer`、`capabilityKey`、`input` 为必备核心字段，其余字段仅作为扩展元信息存在。
- Result 最小公共形状：`status`、`output`、`artifacts`、`evidence`、`error`、`usage`、`metadata` 为核心字段；`executionId/resultId/completedAt` 属于执行级 envelope 外壳。
- Gateway/dispatch 最小入口：`acquire` → `prepare` → `dispatch` + `cancel` + `onResult/onBackpressure`。

当前首轮代码侧冻结锚点如下：

- `src/rax/types.ts`
  - `CapabilitySupportMatrix`
  - `CapabilityRequestCore`
